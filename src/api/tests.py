...
class CommandHandlerTests(APITestCase):
    ...
    def test_missing_header(self):
        data = { 'id': 123 }
        response = self.client.post(reverse('api:commands'), data, format='json')
    ...
    def test_invalid_header(self):
        data = { 'id': 123 }
        response = self.client.post(reverse('api:commands'), data, format='json', HTTP_COMMAND='unknown-command')
        self.assertEquals(response.status_code, status.HTTP_400_BAD_REQUEST)
